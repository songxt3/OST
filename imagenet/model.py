...
class SPS(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.down_sample_conv = nn.Conv2d(embed_dims//8, embed_dims//8, kernel_size=3, stride=2, padding=1, bias=False)
        self.down_sample_bn = nn.BatchNorm2d(embed_dims//8)
        self.down_sample_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')

        self.proj_conv1 = nn.Conv2d(embed_dims//8, embed_dims//4, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn1 = nn.BatchNorm2d(embed_dims//4)
        self.proj_lif1 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.down_sample_conv1 = nn.Conv2d(embed_dims//4, embed_dims//4, kernel_size=3, stride=2, padding=1, bias=False)
        self.down_sample_bn1 = nn.BatchNorm2d(embed_dims//4)
        self.down_sample_lif1 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')

        self.proj_conv2 = nn.Conv2d(embed_dims//4, embed_dims//2, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn2 = nn.BatchNorm2d(embed_dims//2)
        self.proj_lif2 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.down_sample_conv2 = nn.Conv2d(embed_dims//2, embed_dims//2, kernel_size=3, stride=2, padding=1, bias=False)
        self.down_sample_bn2 = nn.BatchNorm2d(embed_dims//2)
        self.down_sample_lif2 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')

        self.proj_conv3 = nn.Conv2d(embed_dims//2, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn3 = nn.BatchNorm2d(embed_dims)
        self.proj_lif3 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.down_sample_conv3 = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=2, padding=1, bias=False)
        self.down_sample_bn3 = nn.BatchNorm2d(embed_dims)
        self.down_sample_lif3 = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
    ...
    def forward(self, x):
        ...
        x = self.down_sample_bn(x).reshape(T, B, -1, H//2, W//2).contiguous()
        x = self.down_sample_lif(x).flatten(0, 1).contiguous()
        ...
        x = self.down_sample_conv1(x)
        x = self.down_sample_bn1(x).reshape(T, B, -1, H//4, W//4).contiguous()
        x = self.down_sample_lif1(x).flatten(0, 1).contiguous()

        x = self.proj_conv2(x)
        x = self.proj_bn2(x).reshape(T, B, -1, H//4, W//4).contiguous()
        x = self.proj_lif2(x).flatten(0, 1).contiguous()

        x = self.down_sample_conv2(x)
        x = self.down_sample_bn2(x).reshape(T, B, -1, H//8, W//8).contiguous()
        x = self.down_sample_lif2(x).flatten(0, 1).contiguous()

        x = self.proj_conv3(x)
        x = self.proj_bn3(x).reshape(T, B, -1, H//8, W//8).contiguous()
        x = self.proj_lif3(x).flatten(0, 1).contiguous()

        x = self.down_sample_conv3(x)
        x = self.down_sample_bn3(x).reshape(T, B, -1, H//16, W//16).contiguous()
        x = self.down_sample_lif3(x).flatten(0, 1).contiguous()
...
class Spikformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T = 6
                 ):
        super().__init__()
        self.T = T  # time step
        ...
        block = nn.ModuleList([Block(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios, block_num=j, depths=depths)
